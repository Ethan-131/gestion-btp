...
async function boot(db) {
  ...
  async function renderStats(root) {
    root.innerHTML ='<div class="v66-pagehead"><div><h1>Statistiques chantiers</h1><p>Temps écoulé, heures réalisées et salariés mobilisés.</p></div></div><div class="v66-project-category-tabs" id="v66StatsCategoryTabs"></div><input class="v66-search" id="v66StatsSearch" placeholder="Code ou nom du chantier…"><div id="v66Stats" class="v66-list" style="margin-top:12px"><div class="v66-card v66-empty">Calcul…</div></div>';
    try {
      const [{ data: projects, error: pe }, { data: sheets, error: se }] =
        await Promise.all([
          ...
          db.from("timesheets").select("employee_id,profiles!timesheets_employee_id_fkey(first_name,last_name,email),timesheet_days(timesheet_sites(project_id,hours))"),
        ]);
      if (pe) throw pe;
      if (se) throw se;
      const actual=new Map(),employeesByProject=new Map();
      (sheets||[]).forEach(sheet=>(sheet.timesheet_days||[]).forEach(day=>(day.timesheet_sites||[]).forEach(site=>{if(!site.project_id)return;const hours=Number(site.hours||0);actual.set(site.project_id,(actual.get(site.project_id)||0)+hours);if(!employeesByProject.has(site.project_id))employeesByProject.set(site.project_id,new Map());const people=employeesByProject.get(site.project_id),employee=people.get(sheet.employee_id)||{name:fullName(sheet.profiles)||sheet.profiles?.email||"Salarié",hours:0};employee.hours+=hours;people.set(sheet.employee_id,employee)})));
      let activeCategory="active",searchValue="";const tabs=root.querySelector("#v66StatsCategoryTabs");
      const paint = () => {
        const rows = projects.filter((p) =>projectTimeCategory(p)===activeCategory&&smartSearchMatch(`${p.code} ${p.name}`,searchValue));
        tabs.innerHTML=["active","upcoming","completed"].map(category=>`<button type="button" class="${category===activeCategory?"active":""}" data-stats-category="${category}">${projectCategoryLabels[category]} <b>${projects.filter(project=>projectTimeCategory(project)===category).length}</b></button>`).join("");tabs.querySelectorAll("[data-stats-category]").forEach(button=>button.onclick=()=>{activeCategory=button.dataset.statsCategory;paint()});
        root.querySelector("#v66Stats").innerHTML = rows.length
          ? rows
              .map((p) => {
                const used=actual.get(p.id)||0,start=new Date(`${p.planned_start_date}T12:00:00`),end=new Date(`${p.planned_end_date}T12:00:00`),today=new Date(),duration=Math.max(1,end-start),timePct=Math.max(0,Math.min(100,((today-start)/duration)*100)),people=[...(employeesByProject.get(p.id)?.values()||[])].sort((a,b)=>b.hours-a.hours),conductors=(p.project_conductors||[]).map(item=>fullName(item.profiles)).filter(Boolean).join(", ")||"Aucun conducteur affecté";
                return `<article class="v66-card v66-project-stat-card"><div class="v66-pagehead"><div><strong>${esc(p.code)} — ${esc(p.name)}</strong><p>${fmtDate(p.planned_start_date)} → ${fmtDate(p.planned_end_date)}</p></div><span class="v66-pill ${projectTimeCategory(p)}">${projectCategoryLabels[projectTimeCategory(p)]}</span></div><div class="v66-project-stat-summary"><div><small>Conducteur de travaux</small><strong>${esc(conductors)}</strong></div><div><small>Heures réalisées</small><strong>${used.toLocaleString("fr-FR")} h</strong></div><div><small>Temps prévisionnel écoulé</small><strong>${timePct.toFixed(0)} %</strong></div></div><div class="v66-time-progress"><span style="width:${timePct}%"></span></div><div class="v66-time-legend"><small>${fmtDate(p.planned_start_date)}</small><b>${timePct.toFixed(0)} % de la période écoulée</b><small>${fmtDate(p.planned_end_date)}</small></div><div class="v66-project-workers"><h3>Salariés ayant travaillé sur ce chantier</h3>${people.length?people.map((person,index)=>`<div class="${index>=3?"v66-worker-extra":""}" ${index>=3?"hidden":""}><span><b>${index+1}</b>${esc(person.name)}</span><strong>${person.hours.toLocaleString("fr-FR")} h</strong></div>`).join(""):'<p class="v66-help">Aucune heure enregistrée.</p>'}${people.length>3?'<button type="button" class="v66-link-btn" data-more-workers>Voir les autres salariés</button>':""}</div></article>`;
              })
              .join("")
          : '<div class="v66-card v66-empty">Aucun chantier trouvé.</div>';
        root.querySelectorAll("[data-more-workers]").forEach(button=>button.onclick=()=>{const card=button.closest(".v66-project-workers"),open=button.dataset.open==="true";card.querySelectorAll(".v66-worker-extra").forEach(row=>row.hidden=open);button.dataset.open=String(!open);button.textContent=open?"Voir les autres salariés":"Réduire"});
      };
      const intended=routeIntent?.projectId?projects.find(p=>p.id===routeIntent.projectId):null;
      searchValue=intended?intended.code:"";if(intended)activeCategory=projectTimeCategory(intended);routeIntent=null;root.querySelector("#v66StatsSearch").value=searchValue;paint();
      root.querySelector("#v66StatsSearch").oninput = (e) =>{searchValue=e.target.value;paint()};
    } catch (e) {
