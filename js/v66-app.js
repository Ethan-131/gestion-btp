...
async function boot(db){
  ...
  function allowedPages(){
    ...
    if(['conducteur','rh','admin'].includes(role))pages.push(['projects','Chantiers']);
    if(['conducteur','rh','admin'].includes(role))pages.push(['stats','Avancement']);
  ...
  async function renderProjects(root){
    ...
    try{
      ...
      const paint=q=>{const filtered=data.filter(p=>`${p.code} ${p.name}`.toLowerCase().includes(q.toLowerCase()));root.querySelector('#v66Projects').innerHTML=filtered.length?filtered.map(p=>{const assigned=(p.project_conductors||[]).some(x=>x.conductor_id===profile.id),canEdit=['rh','admin'].includes(role)||(role==='conducteur'&&assigned);return `<article class="v66-card v66-row" data-id="${p.id}"><div><strong>${esc(p.code)} — ${esc(p.name)}</strong><small>${fmtDate(p.planned_start_date)} → ${fmtDate(p.planned_end_date)} · ${esc(p.planned_days)} jours / ${esc(p.planned_hours)} h</small></div><div><span class="v66-pill ${esc(p.status)}">${esc(statusLabels[p.status]||p.status)}</span><small>${p.project_conductors?.length?esc(p.project_conductors.map(x=>fullName(x.profiles)).join(', ')):'Aucun conducteur'}</small></div><div class="v66-actions">${canEdit?'<button class="v66-btn" data-edit>Modifier</button>':'<span class="v66-help">Consultation</span>'}</div></article>`}).join(''):'<div class="v66-card v66-empty">Aucun chantier trouvé.</div>';root.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>projectModal(data.find(x=>x.id===b.closest('[data-id]').dataset.id)))};paint('');root.querySelector('#v66ProjectSearch').oninput=e=>paint(e.target.value);
