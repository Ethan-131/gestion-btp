...
async function boot(db) {
  ...
  async function renderNativeCurrentTimesheet(panel,year,week){
    ...
    form.querySelector(".v66-template-apply").onclick=()=>{
      const project=form.querySelector(".v66-template-project").value.trim(),hours=Number(form.querySelector(".v66-template-hours").value||0),friday=Number(form.querySelector(".v66-template-friday").value||0),meal=form.querySelector(".v66-template-meal").value,task=form.querySelector(".v66-template-task").value.trim();
      if(!project)return toast("Choisis ou saisis d’abord un chantier dans la journée type.");
