...
const roleLabels = {
  ...
  rh: "RH / Direction",import { createClient } from "https://esm.sh/@supabase/supabase-js@2";
...
async function boot(db) {
  ...
  function renderLeaveCalendar(node, first, last, requests, role) {
    ...
    for (let day = 1; day <= last.getUTCDate(); day++) {
      ...
      cells.push(
        `<div class="v66-cal-day ${weekend ? "weekend" : ""} ${holiday ? "holiday" : ""}"><b>${day}</b>${holiday ? `<span class="v66-holiday">${esc(holiday)}</span>` : ""}${people.map((x) => `<span class="v66-absence">${esc(x)}</span>`).join("")}</div>`,
      );
    }
    node.innerHTML = `${["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((x) => `<div class="v66-cal-label">${x}</div>`).join("")}${cells.join("")}`;
  }

  async function leaveModal(createdByRh = false) {
    let employees = [];
    if (createdByRh) {
      const { data, error } = await db
        .from("profiles")
...
async function boot(db) {
  ...
  async function renderLeaves(root, monthOffset = 0) {
    const role = visibleRole(),
      canReview = role === "rh",
      canCreate = role === "salarie" || canReview;
    const month = new Date();
    month.setUTCDate(1);
    month.setUTCMonth(month.getUTCMonth() + monthOffset);
    const first = new Date(
