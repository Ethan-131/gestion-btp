...
async function boot(db) {
  ...
  async function renderStats(root) {
    ...
    try {
      ...
      const openDrawer=(project,data,label="Toutes périodes")=>{const start=new Date(`${project.planned_start_date}T12:00:00`),end=new Date(`${project.planned_end_date}T12:00:00`),pct=Math.max(0,Math.min(100,((new Date()-start)/Math.max(1,end-start))*100)),people=data.people.filter(x=>x.projectId===project.id),drawer=el("div",{class:"v66-drawer-overlay"},`<article class="v66-side-drawer v66-project-stats-drawer"><header><div><small>${esc(project.code)}</small><h2>${esc(project.name)}</h2><span class="v66-pill ${projectTimeCategory(project)}">${projectCategoryLabels[projectTimeCategory(project)]}</span></div><button type="button" class="v66-icon-button" data-close>×</button></header><main><p class="v66-help">${esc(label)}</p><div class="v66-project-stat-summary"><div><small>Conducteur</small><strong>${esc(conductor(project))}</strong></div><div><small>Heures réalisées</small><strong>${(data.byProject.get(project.id)||0).toLocaleString("fr-FR")} h</strong></div><div><small>Période écoulée</small><strong>${pct.toFixed(0)} %</strong></div></div><div class="v66-time-progress"><span style="width:${pct}%"></span></div><div class="v66-time-legend"><small>${fmtDate(project.planned_start_date)}</small><b>${pct.toFixed(0)} %</b><small>${fmtDate(project.planned_end_date)}</small></div><section class="v66-project-workers"><h3>Temps par salarié</h3>${people.length?people.map((person,index)=>`<div class="${index>=3?"v66-worker-extra":""}" ${index>=3?"hidden":""}><span><b>${index+1}</b>${esc(person.name)}</span><strong>${person.hours.toLocaleString("fr-FR")} h</strong></div>`).join(""):'<p class="v66-help">Aucune heure enregistrée.</p>'}${people.length>3?'<button type="button" class="v66-link-btn" data-more-workers>Voir les autres salariés</button>':""}</section></main></article>`);document.body.appendChild(drawer);drawer.querySelector("[data-close]").onclick=()=>drawer.remove();drawer.onclick=e=>{if(e.target===drawer)drawer.remove()};drawer.querySelector("[data-more-workers]")?.addEventListener("click",e=>{const hidden=[...drawer.querySelectorAll(".v66-worker-extra")].some(row=>row.hidden);drawer.querySelectorAll(".v66-worker-extra").forEach(row=>row.hidden=!hidden);e.currentTarget.textContent=hidden?"Réduire":"Voir les autres salariés"})};
