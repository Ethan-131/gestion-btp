...
async function boot(db) {
  ...
  function renderProjectHub(root) {
    const intent=routeIntent, requested=intent?.projectTab;
    let active=requested==="stats"?"analysis":"pilotage"; routeIntent=null;
    root.innerHTML=`<div class="v105-project-shell"><div class="v66-pagehead v105-module-head"><div><span class="v105-eyebrow">PILOTAGE OPÉRATIONNEL</span><h1>Chantiers</h1><p>Suivez l’activité, repérez les chantiers à surveiller et ouvrez leur fiche détaillée.</p></div></div><nav class="v105-project-nav" aria-label="Navigation chantiers"><button type="button" data-project-tab="pilotage">Pilotage</button><button type="button" data-project-tab="analysis">Analyse</button></nav><section id="v66ProjectContent"></section></div>`;
    const content=root.querySelector("#v66ProjectContent");
    const show=(tab)=>{active=tab;root.querySelectorAll("[data-project-tab]").forEach(b=>b.classList.toggle("active",b.dataset.projectTab===tab));if(tab==="analysis")renderStats(content);else renderProjects(content)};
    root.querySelectorAll("[data-project-tab]").forEach(b=>b.onclick=()=>show(b.dataset.projectTab));
    show(active);
  }
  ...
  const projectCategoryLabels={active:"En cours",upcoming:"À venir",completed:"Terminé"};
  const v105FmtHours=value=>`${Number(value||0).toLocaleString("fr-FR",{maximumFractionDigits:1})} h`;
  function v105ProjectTimePct(project, reference=new Date()){
    if(!project?.planned_start_date||!project?.planned_end_date)return 0;
    const start=new Date(`${project.planned_start_date}T12:00:00`),end=new Date(`${project.planned_end_date}T12:00:00`),now=reference;
    if(!Number.isFinite(start.getTime())||!Number.isFinite(end.getTime())||end<=start)return 0;
    return Math.max(0,Math.min(100,((now-start)/(end-start))*100));
  }
  function v105Health(project,actual){
    const category=projectTimeCategory(project), planned=Number(project?.planned_hours||0), hoursPct=planned>0?actual/planned*100:null, timePct=v105ProjectTimePct(project);
    if(category==="upcoming")return{tone:"upcoming",label:"À venir",text:"Le chantier n’a pas encore commencé.",hoursPct,timePct};
    if(planned<=0)return{tone:"neutral",label:"À renseigner",text:"Ajoutez les heures prévues pour activer le suivi.",hoursPct,timePct};
    if(hoursPct>=100.01)return{tone:"danger",label:"Dépassement",text:`${Math.round(hoursPct)} % des heures prévues consommées.`,hoursPct,timePct};
    if(category==="completed")return{tone:"good",label:"Terminé",text:`Terminé à ${Math.round(hoursPct)} % du prévisionnel horaire.`,hoursPct,timePct};
    const gap=hoursPct-timePct;
    if(timePct>=15&&gap>=15)return{tone:"warning",label:"À surveiller",text:`Les heures sont consommées ${Math.round(gap)} points plus vite que le temps.`,hoursPct,timePct};
    return{tone:"good",label:"Situation normale",text:"La consommation d’heures reste cohérente avec le planning.",hoursPct,timePct};
  }

  async function renderProjects(root) {
    const role=visibleRole();
    root.innerHTML=`<section class="v105-pilotage-head"><div><h2>Vue d’ensemble</h2><p>Une lecture rapide de l’état des chantiers.</p></div><button class="v66-btn primary" id="v66NewProject">+ Nouveau chantier</button></section><div id="v105PortfolioKpis" class="v105-portfolio-kpis"><div class="v105-skeleton"></div><div class="v105-skeleton"></div><div class="v105-skeleton"></div><div class="v105-skeleton"></div></div><section class="v105-project-toolbar"><div class="v105-search-wrap"><span>⌕</span><input class="v66-search" id="v66ProjectSearch" placeholder="Rechercher un chantier…"></div><div class="v105-filter-row" id="v66ProjectCategoryTabs"></div></section><div class="v105-project-list" id="v66Projects"><div class="v66-card v66-empty">Chargement des chantiers…</div></div>`;
    root.querySelector("#v66NewProject").onclick=()=>projectModal();
    try{
      const [{data,error},{data:sites,error:siteError}]=await Promise.all([
      ...
      const paint=()=>{
        paintKpis();
        const counts=Object.fromEntries(["active","upcoming","completed"].map(c=>[c,data.filter(p=>projectTimeCategory(p)===c).length]));
        tabs.innerHTML=["active","upcoming","completed"].map(c=>`<button type="button" class="${c===activeCategory?"active":""}" data-project-category="${c}">${projectCategoryLabels[c]} <b>${counts[c]}</b></button>`).join("");
        ...
        list.querySelectorAll("[data-edit-project]").forEach(b=>b.onclick=e=>{e.stopPropagation();projectModal(data.find(x=>x.id===b.dataset.editProject))});
      };
      root.querySelector("#v66ProjectSearch").oninput=e=>{searchValue=e.target.value;paint()};paint();
  ...
  async function projectModal(project = null) {
    ...
    const startInput=modal.querySelector('[name="planned_start_date"]'),endInput=modal.querySelector('[name="planned_end_date"]'),plannedHoursInput=modal.querySelector('[name="planned_hours"]'),periodLabel=modal.querySelector("[data-project-period-label]");
    const estimatedHours=()=>{if(!startInput.value||!endInput.value||endInput.value<startInput.value)return 0;let total=0,cursor=new Date(`${startInput.value}T12:00:00`),last=new Date(`${endInput.value}T12:00:00`);while(cursor<=last){const day=cursor.getDay();if(day>=1&&day<=4)total+=8;else if(day===5)total+=7;cursor.setDate(cursor.getDate()+1)}return total};
    let plannedHoursManual=!!project?.planned_hours;plannedHoursInput.oninput=()=>{plannedHoursManual=true};
    const refreshPeriodLabel=()=>{periodLabel.textContent=startInput.value&&endInput.value?`Du ${fmtDate(startInput.value)} au ${fmtDate(endInput.value)}`:startInput.value?`Début le ${fmtDate(startInput.value)} · choisissez la fin`:"Aucune période sélectionnée";if(!plannedHoursManual&&startInput.value&&endInput.value)plannedHoursInput.value=String(estimatedHours()||"")};startInput.onchange=refreshPeriodLabel;endInput.onchange=refreshPeriodLabel;
    ...
    modal.querySelector("form").onsubmit = async (e) => {
      ...
      const values = {
        ...
        planned_hours: Math.max(0, Number(fd.get("planned_hours") || 0)),
  ...
  async function renderStats(root) {
    root.innerHTML=`<section class="v105-analysis"><div class="v105-analysis-head"><div><span class="v105-eyebrow">ANALYSE CHANTIERS</span><h2>Analyse</h2><p>Analysez une période ou ouvrez la fiche complète d’un chantier.</p></div><button type="button" class="v66-btn" data-back-pilotage>← Pilotage</button></div><div class="v105-analysis-tabs"><button type="button" class="active" data-stats-view="general">Vue générale</button><button type="button" data-stats-view="projects">Par chantier</button></div><div id="v66Stats"><div class="v66-card v66-empty">Calcul des indicateurs…</div></div></section>`;
    root.querySelector("[data-back-pilotage]").onclick=()=>renderProjects(root);
    try{
      const [{data:projects,error:pe},{data:sheets,error:se}]=await Promise.all([
        db.from("projects").select("*,project_conductors(conductor_id,profiles!project_conductors_conductor_id_fkey(first_name,last_name))").order("code"),
        db.from("timesheets").select("employee_id,status,profiles!timesheets_employee_id_fkey(first_name,last_name,email),timesheet_days(work_date,travel_km,it_needs_review,timesheet_sites(project_id,hours))")
      ]);if(pe)throw pe;if(se)throw se;
      const entries=[],itEntries=[];
      (sheets||[]).forEach(sheet=>(sheet.timesheet_days||[]).forEach(day=>{
        const daySites=(day.timesheet_sites||[]).filter(site=>site.project_id);const projectsToday=[...new Set(daySites.map(site=>site.project_id))];
        daySites.forEach(site=>entries.push({projectId:site.project_id,date:day.work_date,employeeId:sheet.employee_id,name:fullName(sheet.profiles)||sheet.profiles?.email||"Salarié",hours:Number(site.hours||0)}));
        if(projectsToday.length===1&&Number(day.travel_km||0)>0)itEntries.push({projectId:projectsToday[0],date:day.work_date,employeeId:sheet.employee_id,km:Number(day.travel_km||0),needsReview:!!day.it_needs_review});
      }));
      const periodBounds=(year,month="all",cutoffMonthDay=null)=>{const y=Number(year),m=month==="all"?null:Number(month),start=m===null?`${y}-01-01`:`${y}-${String(m+1).padStart(2,"0")}-01`;let endDate=m===null?new Date(Date.UTC(y,11,31)):new Date(Date.UTC(y,m+1,0));const today=new Date(),currentYear=today.getFullYear();if(y===currentYear){const todayUtc=new Date(Date.UTC(y,today.getMonth(),today.getDate()));if(todayUtc<endDate)endDate=todayUtc}if(cutoffMonthDay&&m===null){const cutoff=new Date(Date.UTC(y,cutoffMonthDay.month,cutoffMonthDay.day));if(cutoff<endDate)endDate=cutoff}const partial=m===null&&dateKey(endDate)<`${y}-12-31`,label=m===null?`Année ${y}${partial?` · jusqu’au ${endDate.toLocaleDateString("fr-FR",{day:"numeric",month:"long",timeZone:"UTC"})}`:""}`:`${monthLabels[m]} ${y}`;return{year:y,month:m,start,end:dateKey(endDate),label,partial}};
      const inPeriod=(date,p)=>date>=p.start&&date<=p.end;
      const projectOverlaps=(p,period)=>String(p.planned_start_date||"")<=period.end&&String(p.planned_end_date||"")>=period.start;
      const metrics=(projectId="all",month="all",year=new Date().getFullYear(),cutoffMonthDay=null)=>{
        const period=periodBounds(year,month,cutoffMonthDay),filtered=entries.filter(x=>(projectId==="all"||x.projectId===projectId)&&inPeriod(x.date,period)),byProject=new Map(),byEmployee=new Map(),byProjectEmployee=new Map(),buckets=new Map(),employeeDays=new Set(),activeDays=new Set();
        filtered.forEach(x=>{byProject.set(x.projectId,(byProject.get(x.projectId)||0)+x.hours);const emp=byEmployee.get(x.employeeId)||{employeeId:x.employeeId,name:x.name,hours:0};emp.hours+=x.hours;byEmployee.set(x.employeeId,emp);const k=`${x.projectId}:${x.employeeId}`,pe=byProjectEmployee.get(k)||{projectId:x.projectId,employeeId:x.employeeId,name:x.name,hours:0};pe.hours+=x.hours;byProjectEmployee.set(k,pe);employeeDays.add(`${x.employeeId}:${x.date}`);activeDays.add(`${x.projectId}:${x.date}`);const d=new Date(`${x.date}T12:00:00Z`),bucket=month==="all"?monthLabels[d.getUTCMonth()]:`S${String(isoWeekFromDate(d).week).padStart(2,"0")}`;buckets.set(bucket,(buckets.get(bucket)||0)+x.hours)});
        const total=filtered.reduce((s,x)=>s+x.hours,0),employees=byEmployee.size,projectCount=byProject.size,it=itEntries.filter(x=>(projectId==="all"||x.projectId===projectId)&&inPeriod(x.date,period));
        const orderedBuckets=new Map([...buckets.entries()].sort((a,b)=>month==="all"?monthLabels.indexOf(a[0])-monthLabels.indexOf(b[0]):Number(String(a[0]).replace("S",""))-Number(String(b[0]).replace("S",""))));
        return{period,total,employees,projectCount,workdays:employeeDays.size,activeDays:activeDays.size,avgProject:projectCount?total/projectCount:0,avgEmployee:employees?total/employees:0,byProject,people:[...byProjectEmployee.values()].sort((a,b)=>b.hours-a.hours),employeeRanking:[...byEmployee.values()].sort((a,b)=>b.hours-a.hours),buckets:orderedBuckets,itTotal:it.reduce((s,x)=>s+x.km,0),itReview:it.filter(x=>x.needsReview).length,statusCounts:{started:projects.filter(p=>inPeriod(String(p.planned_start_date||""),period)).length,completed:projects.filter(p=>projectTimeCategory(p)==="completed"&&inPeriod(String(p.planned_end_date||""),period)).length,active:projects.filter(p=>projectTimeCategory(p)==="active"&&projectOverlaps(p,period)).length}};
      };
      const conductor=p=>(p.project_conductors||[]).map(x=>fullName(x.profiles)).filter(Boolean).join(", ")||"Aucun conducteur";
      const fmt=v105FmtHours,now=new Date(),stats=root.querySelector("#v66Stats");
      const years=()=>{const current=now.getFullYear();return Array.from({length:Math.max(1,current-2024+1)},(_,i)=>2024+i).map(y=>`<option value="${y}">${y}</option>`).join("")};
      const months=()=>`<option value="all">Toute l’année</option>${monthLabels.map((m,i)=>`<option value="${i}">${m}</option>`).join("")}`;
      const delta=(a,b,suffix="")=>{const diff=Number(a||0)-Number(b||0),pct=Number(b)?diff/Number(b)*100:null;return `<b class="${diff>0?"up":diff<0?"down":"flat"}">${diff>0?"+":""}${Number(diff.toFixed(1)).toLocaleString("fr-FR")}${suffix}</b><small>${pct===null?(Number(a)?"nouveau":"0 %"):`${pct>0?"+":""}${pct.toFixed(1).replace(".",",")} %`}</small>`};
      const projection=(project,actual)=>{const planned=Number(project.planned_hours||0),timePct=v105ProjectTimePct(project);if(projectTimeCategory(project)==="completed")return actual;if(!planned||timePct<15||actual<40)return null;return actual/(timePct/100)};
      const dailyForProject=(projectId)=>{const rows=entries.filter(x=>x.projectId===projectId),days=new Map();rows.forEach(x=>{const d=days.get(x.date)||{hours:0,employees:new Set()};d.hours+=x.hours;d.employees.add(x.employeeId);days.set(x.date,d)});return days};
      const weeklyForProject=(projectId)=>{const rows=entries.filter(x=>x.projectId===projectId),weeks=new Map();rows.forEach(x=>{const d=new Date(`${x.date}T12:00:00Z`),w=isoWeekFromDate(d),key=`${w.year}-${String(w.week).padStart(2,"0")}`,cur=weeks.get(key)||{label:`S${w.week}`,hours:0};cur.hours+=x.hours;weeks.set(key,cur)});return [...weeks.values()]};
      const barChart=(rows)=>{if(!rows.length)return'<div class="v66-empty">Aucune heure enregistrée.</div>';const max=Math.max(...rows.map(x=>x.hours),1);return`<div class="v105-bars">${rows.slice(-16).map(x=>`<div class="v105-bar"><div><i style="height:${Math.max(4,x.hours/max*100)}%"></i></div><small>${esc(x.label)}</small><b>${Number(x.hours).toLocaleString("fr-FR",{maximumFractionDigits:0})}</b></div>`).join("")}</div>`};
      ...
      const openProject=(project)=>{
        ...
        const overview=()=>{body.innerHTML=`<section class="v105-primary-progress"><div class="v105-primary-numbers"><span><small>Heures réalisées</small><strong>${fmt(actual)}</strong></span><span><small>Heures prévues</small><strong>${planned?fmt(planned):"—"}</strong></span><span><small>Restantes</small><strong>${planned?fmt(remaining):"—"}</strong></span></div><div class="v105-big-progress"><i class="${health.tone}" style="width:${Math.min(100,Math.max(0,hoursPct||0))}%"></i></div><div class="v105-primary-foot"><b>${hoursPct!==null?`${Math.round(hoursPct)} % consommé`:"Prévisionnel à renseigner"}</b><span>${health.text}</span></div></section><section class="v105-two-col"><article class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">↗</span><h3>Planning vs heures</h3></div></div><div class="v105-dual-progress"><div><span>Temps écoulé</span><b>${Math.round(timePct)} %</b><i><em style="width:${timePct}%"></em></i></div><div><span>Heures consommées</span><b>${hoursPct!==null?Math.round(hoursPct)+" %":"—"}</b><i><em class="${health.tone}" style="width:${Math.min(100,Math.max(0,hoursPct||0))}%"></em></i></div></div>${gap!==null&&projectTimeCategory(project)==="active"?`<p class="v105-insight ${health.tone}">${gap>10?"⚠":"✓"} ${gap>0?`Les heures avancent de ${Math.round(gap)} points plus vite que le temps.`:`La consommation d’heures reste maîtrisée par rapport au planning.`}</p>`:""}</article><article class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">⌁</span><h3>Projection fin de chantier</h3></div></div>${proj!==null&&planned?`<div class="v105-projection"><strong>≈ ${fmt(proj)}</strong><span>au rythme actuel</span><div class="${proj>planned?"danger":"good"}">${proj>planned?`+${fmt(proj-planned)} estimées`:`${fmt(planned-proj)} de marge estimée`}</div></div>`:`<div class="v105-empty-soft"><strong>Projection bientôt disponible</strong><span>Elle apparaît après suffisamment d’activité sur le chantier.</span></div>`}</article></section><section class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">⌁</span><h3>Heures cumulées</h3></div><small>Prévisionnel vs réel</small></div>${cumulativeChart(project,weekly)}</section><section class="v105-alert-card ${health.tone}"><div><span>${health.tone==="danger"?"!":health.tone==="warning"?"⚠":"✓"}</span><div><strong>${health.label}</strong><p>${esc(health.text)}</p></div></div></section>`};
        const team=()=>{body.innerHTML=`<div class="v105-stat-grid"><article><small>Salariés intervenus</small><strong>${people.length}</strong><span>depuis le début</span></article><article><small>Effectif moyen</small><strong>${avgEffectif.toLocaleString("fr-FR",{maximumFractionDigits:1})}</strong><span>par jour actif</span></article><article><small>Effectif maximum</small><strong>${maxEffectif}</strong><span>sur une journée</span></article><article><small>Journées-personnes</small><strong>${dayRows.reduce((s,[,d])=>s+d.employees.size,0)}</strong><span>présences cumulées</span></article></div><section class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">👥</span><h3>Temps par salarié</h3></div></div>${canSeePeople?(people.length?`<div class="v105-ranking">${people.map((p,i)=>`<div><b>${i+1}</b><span>${esc(p.name)}</span><strong>${fmt(p.hours)}</strong></div>`).join("")}</div>`:'<div class="v66-empty">Aucune heure enregistrée.</div>'):'<div class="v105-empty-soft"><strong>Donnée réservée</strong><span>Le détail nominatif est visible pour les conducteurs, RH et administrateurs.</span></div>'}</section>`};
        const activity=()=>{body.innerHTML=`<div class="v105-stat-grid"><article><small>Jours actifs</small><strong>${days.size}</strong><span>jours avec des heures</span></article><article><small>Première activité</small><strong>${first?fmtDate(first):"—"}</strong><span>première heure saisie</span></article><article><small>Dernière activité</small><strong>${last?fmtDate(last):"—"}</strong><span>dernière heure saisie</span></article><article><small>Semaine la + chargée</small><strong>${peak?peak.label:"—"}</strong><span>${peak?fmt(peak.hours):"aucune donnée"}</span></article></div><section class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">▥</span><h3>Heures par semaine</h3></div><small>${weekly.length} semaine${weekly.length>1?"s":""} avec activité</small></div>${barChart(weekly)}</section></section>`};
        const itTab=()=>{body.innerHTML=`<div class="v105-stat-grid"><article><small>IT total déclaré</small><strong>${itTotal.toLocaleString("fr-FR",{maximumFractionDigits:0})} km</strong><span>jours attribuables à ce chantier</span></article><article><small>IT moyen / journée</small><strong>${itAvg.toLocaleString("fr-FR",{maximumFractionDigits:1})} km</strong><span>${projectIt.length} journée${projectIt.length>1?"s":""}</span></article><article><small>IT à vérifier</small><strong>${projectIt.filter(x=>x.needsReview).length}</strong><span>signalements</span></article><article><small>IT théorique</small><strong>${Number(project.it_km_plaisance||0).toLocaleString("fr-FR")} / ${Number(project.it_km_salies||0).toLocaleString("fr-FR")}</strong><span>Plaisance / Salies</span></article></div><section class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">↔</span><h3>Historique IT</h3></div></div>${projectIt.length?`<div class="v105-it-list">${projectIt.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,30).map(x=>`<div><span>${fmtDate(x.date)}</span><strong>${x.km.toLocaleString("fr-FR")} km</strong>${x.needsReview?'<b>À vérifier</b>':'<small>Automatique / validé</small>'}</div>`).join("")}</div>`:'<div class="v66-empty">Aucun IT attribuable automatiquement à ce chantier.</div>'}</section>`};
        const views={overview,team,activity,it:itTab};root.querySelectorAll("[data-detail-tab]").forEach(b=>b.onclick=()=>{root.querySelectorAll("[data-detail-tab]").forEach(x=>x.classList.toggle("active",x===b));views[b.dataset.detailTab]()});root.querySelector("[data-detail-back]").onclick=()=>renderStats(root);root.querySelector("[data-detail-edit]").onclick=()=>projectModal(project);overview();window.scrollTo({top:0,behavior:"smooth"});
      };

      const renderGeneral=()=>{
        stats.innerHTML=`<section class="v105-period-toolbar"><div><label>Période<select id="v66StatsMonth">${months()}</select></label><label>Année<select id="v66StatsYear">${years()}</select></label></div><button type="button" class="v66-btn" id="v66StatsCurrent">Année actuelle</button><button type="button" class="v66-btn primary" id="v105Compare">Comparer deux années</button></section><div id="v105Overview"></div>`;
        stats.querySelector("#v66StatsMonth").value="all";stats.querySelector("#v66StatsYear").value=String(now.getFullYear());
        let rankingExpanded=false;
        const paint=()=>{const data=metrics("all",stats.querySelector("#v66StatsMonth").value,stats.querySelector("#v66StatsYear").value),ranking=[...data.byProject.entries()].sort((a,b)=>b[1]-a[1]),rankingLimit=rankingExpanded?ranking.length:6,activeProjects=projects.filter(p=>projectTimeCategory(p)==="active"),attention=activeProjects.map(p=>({p,used:entries.filter(x=>x.projectId===p.id).reduce((s,x)=>s+x.hours,0)})).map(x=>({...x,health:v105Health(x.p,x.used)})).filter(x=>["warning","danger"].includes(x.health.tone)).sort((a,b)=>b.health.hoursPct-a.health.hoursPct);root.querySelector("#v105Overview").innerHTML=`<div class="v105-stat-grid"><article><small>Heures réalisées</small><strong>${fmt(data.total)}</strong><span>${esc(data.period.label)}</span></article><article><small>Chantiers travaillés</small><strong>${data.projectCount}</strong><span>avec heures saisies</span></article><article><small>Salariés mobilisés</small><strong>${data.employees}</strong><span>personnes distinctes</span></article><article><small>Journées-personnes</small><strong>${data.workdays}</strong><span>présences cumulées</span></article></div>${attention.length?`<section class="v66-card v105-attention"><div class="v105-card-head"><div><span class="v105-card-icon">!</span><h3>Chantiers nécessitant votre attention</h3></div><small>${attention.length} chantier${attention.length>1?"s":""}</small></div><div>${attention.slice(0,5).map(x=>`<button data-open-project="${x.p.id}"><span class="v105-health ${x.health.tone}"><i></i>${x.health.label}</span><div><strong>${esc(x.p.code)} — ${esc(x.p.name)}</strong><small>${esc(x.health.text)}</small></div><b>›</b></button>`).join("")}</div></section>`:""}<section class="v105-two-col"><article class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">▥</span><h3>Heures par ${data.period.month===null?"mois":"semaine"}</h3></div></div>${barChart([...data.buckets].map(([label,hours])=>({label,hours})))}</article><article class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">⌁</span><h3>Répartition par chantier</h3></div><small>${ranking.length} chantier${ranking.length>1?"s":""}</small></div>${ranking.length?`<div class="v105-ranking">${ranking.slice(0,rankingLimit).map(([id,hours],i)=>{const p=projects.find(x=>x.id===id);return p?`<button data-open-project="${id}"><b>${i+1}</b><span>${esc(p.name)}<small>${data.total?Math.round(hours/data.total*100):0} % des heures</small></span><strong>${fmt(hours)}</strong></button>`:""}).join("")}</div>${ranking.length>6?`<button type="button" class="v105-ranking-toggle" data-ranking-toggle>${rankingExpanded?"Voir moins":`Voir les ${ranking.length-6} suivants`} <span>${rankingExpanded?"−":"+"}</span></button>`:""}`:'<div class="v66-empty">Aucune heure sur cette période.</div>'}</article></section><section class="v66-card v105-period-movements"><div class="v105-card-head"><div><span class="v105-card-icon">↻</span><h3>Mouvements de chantiers</h3></div></div><div><span><small>Démarrés</small><strong>${data.statusCounts.started}</strong></span><span><small>Terminés</small><strong>${data.statusCounts.completed}</strong></span><span><small>En cours</small><strong>${data.statusCounts.active}</strong></span></div></section>`;root.querySelectorAll("[data-open-project]").forEach(b=>b.onclick=()=>openProject(projects.find(p=>p.id===b.dataset.openProject)));const toggle=root.querySelector("[data-ranking-toggle]");if(toggle)toggle.onclick=()=>{rankingExpanded=!rankingExpanded;paint()}};
        stats.querySelectorAll("select").forEach(s=>s.onchange=()=>{rankingExpanded=false;paint()});stats.querySelector("#v66StatsCurrent").onclick=()=>{stats.querySelector("#v66StatsMonth").value="all";stats.querySelector("#v66StatsYear").value=String(now.getFullYear());rankingExpanded=false;paint()};stats.querySelector("#v105Compare").onclick=renderCompare;paint();
      };
      const renderCompare=()=>{
        stats.innerHTML=`<section class="v105-compare-header"><div><h3>Comparer deux années</h3><p>Mesurez l’évolution ou la régression d’une année à l’autre. Si l’une des années est en cours, la comparaison s’arrête automatiquement à la même date dans les deux années.</p></div><button class="v66-btn" data-compare-back>← Vue générale</button></section><div class="v105-compare-pickers"><article><strong>Année A</strong><label>Année<select id="v105YearA">${years()}</select></label></article><article><strong>Année B</strong><label>Année<select id="v105YearB">${years()}</select></label></article></div><div id="v105CompareResults"></div>`;
        const yearA=stats.querySelector("#v105YearA"),yearB=stats.querySelector("#v105YearB"),currentYear=now.getFullYear();yearA.value=String(currentYear);yearB.value=String(Math.max(2024,currentYear-1));
        const paint=()=>{
          const aYear=Number(yearA.value),bYear=Number(yearB.value),hasCurrent=aYear===currentYear||bYear===currentYear,cutoff=hasCurrent?{month:now.getMonth(),day:now.getDate()}:null,A=metrics("all","all",aYear,cutoff),B=metrics("all","all",bYear,cutoff),items=[["Heures réalisées",A.total,B.total,"h"],["Chantiers travaillés",A.projectCount,B.projectCount,""] ,["Salariés mobilisés",A.employees,B.employees,""] ,["Journées-personnes",A.workdays,B.workdays,""] ,["Jours de chantier",A.activeDays,B.activeDays,""] ,["Heures moy. / chantier",A.avgProject,B.avgProject,"h"],["Heures moy. / salarié",A.avgEmployee,B.avgEmployee,"h"],["Chantiers démarrés",A.statusCounts.started,B.statusCounts.started,""] ,["Chantiers terminés",A.statusCounts.completed,B.statusCounts.completed,""] ,["Chantiers en cours",A.statusCounts.active,B.statusCounts.active,""]];
        ...
        stats.querySelectorAll("select").forEach(s=>s.onchange=paint);stats.querySelector("[data-compare-back]").onclick=renderGeneral;paint()
      };
      const renderProjectsView=()=>{stats.innerHTML=`<section class="v105-project-browser"><div class="v105-search-wrap"><span>⌕</span><input class="v66-search" id="v105StatsSearch" placeholder="Rechercher par code, nom ou conducteur…"></div><div id="v105StatsProjects"></div></section>`;let q="";const paint=()=>{const rows=projects.filter(p=>smartSearchMatch(`${p.code} ${p.name} ${conductor(p)}`,q));stats.querySelector("#v105StatsProjects").innerHTML=rows.length?`<div class="v105-project-list">${rows.map(p=>{const actual=entries.filter(x=>x.projectId===p.id).reduce((s,x)=>s+x.hours,0),health=v105Health(p,actual),planned=Number(p.planned_hours||0);return`<button class="v105-project-browser-row" data-project="${p.id}"><div><small>${esc(p.code)}</small><strong>${esc(p.name)}</strong><span>${esc(conductor(p))}</span></div><div><b>${fmt(actual)}</b><small>${planned?`sur ${fmt(planned)}`:"Prévisionnel non renseigné"}</small></div><span class="v105-health ${health.tone}"><i></i>${health.label}</span><em>›</em></button>`}).join("")}</div>`:'<div class="v66-card v66-empty">Aucun chantier trouvé.</div>';stats.querySelectorAll("[data-project]").forEach(b=>b.onclick=()=>openProject(projects.find(p=>p.id===b.dataset.project)))};stats.querySelector("#v105StatsSearch").oninput=e=>{q=e.target.value;paint()};paint()};
      root.querySelectorAll("[data-stats-view]").forEach(b=>b.onclick=()=>{root.querySelectorAll("[data-stats-view]").forEach(x=>x.classList.toggle("active",x===b));b.dataset.statsView==="projects"?renderProjectsView():renderGeneral()});
      const intended=routeIntent?.projectId?projects.find(p=>p.id===routeIntent.projectId):null;routeIntent=null;renderGeneral();if(intended)openProject(intended);
