...
async function boot(db) {
  ...
  async function renderStats(root) {
    ...
    try{
      ...
      const periodBounds=(year,month="all")=>{const y=Number(year),m=month==="all"?null:Number(month),start=m===null?`${y}-01-01`:`${y}-${String(m+1).padStart(2,"0")}-01`,endDate=m===null?new Date(Date.UTC(y,11,31)):new Date(Date.UTC(y,m+1,0));return{year:y,month:m,start,end:dateKey(endDate),label:m===null?`Année ${y}`:`${monthLabels[m]} ${y}`}};
      ...
      const metrics=(projectId="all",month="all",year=new Date().getFullYear())=>{
        const period=periodBounds(year,month),filtered=entries.filter(x=>(projectId==="all"||x.projectId===projectId)&&inPeriod(x.date,period)),byProject=new Map(),byEmployee=new Map(),byProjectEmployee=new Map(),buckets=new Map(),employeeDays=new Set(),activeDays=new Set();
        ...
        return{period,total,employees,projectCount,workdays:employeeDays.size,activeDays:activeDays.size,avgProject:projectCount?total/projectCount:0,avgEmployee:employees?total/employees:0,byProject,people:[...byProjectEmployee.values()].sort((a,b)=>b.hours-a.hours),employeeRanking:[...byEmployee.values()].sort((a,b)=>b.hours-a.hours),buckets,itTotal:it.reduce((s,x)=>s+x.km,0),itReview:it.filter(x=>x.needsReview).length,statusCounts:{started:projects.filter(p=>inPeriod(String(p.planned_start_date||""),period)).length,completed:projects.filter(p=>projectTimeCategory(p)==="completed"&&inPeriod(String(p.planned_end_date||""),period)).length,active:projects.filter(p=>projectTimeCategory(p)==="active"&&projectOverlaps(p,period)).length}};
      };
      ...
      const years=()=>Array.from({length:81},(_,i)=>2020+i).map(y=>`<option value="${y}">${y}</option>`).join("");
      ...
      const renderGeneral=()=>{
        stats.innerHTML=`<section class="v105-period-toolbar"><div><label>Période<select id="v66StatsMonth">${months()}</select></label><label>Année<select id="v66StatsYear">${years()}</select></label></div><button type="button" class="v66-btn" id="v66StatsCurrent">Période actuelle</button><button type="button" class="v66-btn primary" id="v105Compare">Comparer une autre période</button></section><div id="v105Overview"></div>`;
        ...
        const paint=()=>{const data=metrics("all",stats.querySelector("#v66StatsMonth").value,stats.querySelector("#v66StatsYear").value),ranking=[...data.byProject.entries()].sort((a,b)=>b[1]-a[1]),activeProjects=projects.filter(p=>projectTimeCategory(p)==="active"),attention=activeProjects.map(p=>({p,used:entries.filter(x=>x.projectId===p.id).reduce((s,x)=>s+x.hours,0)})).map(x=>({...x,health:v105Health(x.p,x.used)})).filter(x=>["warning","danger"].includes(x.health.tone)).sort((a,b)=>b.health.hoursPct-a.health.hoursPct);root.querySelector("#v105Overview").innerHTML=`<div class="v105-stat-grid"><article><small>Heures réalisées</small><strong>${fmt(data.total)}</strong><span>${esc(data.period.label)}</span></article><article><small>Chantiers travaillés</small><strong>${data.projectCount}</strong><span>avec heures saisies</span></article><article><small>Salariés mobilisés</small><strong>${data.employees}</strong><span>personnes distinctes</span></article><article><small>Journées-personnes</small><strong>${data.workdays}</strong><span>présences cumulées</span></article></div>${attention.length?`<section class="v66-card v105-attention"><div class="v105-card-head"><div><span class="v105-card-icon">!</span><h3>Chantiers nécessitant votre attention</h3></div><small>${attention.length} chantier${attention.length>1?"s":""}</small></div><div>${attention.slice(0,5).map(x=>`<button data-open-project="${x.p.id}"><span class="v105-health ${x.health.tone}"><i></i>${x.health.label}</span><div><strong>${esc(x.p.code)} — ${esc(x.p.name)}</strong><small>${esc(x.health.text)}</small></div><b>›</b></button>`).join("")}</div></section>`:""}<section class="v105-two-col"><article class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">▥</span><h3>Heures par ${data.period.month===null?"mois":"semaine"}</h3></div></div>${barChart([...data.buckets].map(([label,hours])=>({label,hours})))}</article><article class="v66-card"><div class="v105-card-head"><div><span class="v105-card-icon">⌁</span><h3>Répartition par chantier</h3></div></div>${ranking.length?`<div class="v105-ranking">${ranking.slice(0,7).map(([id,hours],i)=>{const p=projects.find(x=>x.id===id);return p?`<button data-open-project="${id}"><b>${i+1}</b><span>${esc(p.name)}<small>${data.total?Math.round(hours/data.total*100):0} % des heures</small></span><strong>${fmt(hours)}</strong></button>`:""}).join("")}</div>`:'<div class="v66-empty">Aucune heure sur cette période.</div>'}</article></section><section class="v66-card v105-period-movements"><div class="v105-card-head"><div><span class="v105-card-icon">↻</span><h3>Mouvements de chantiers</h3></div></div><div><span><small>Démarrés</small><strong>${data.statusCounts.started}</strong></span><span><small>Terminés</small><strong>${data.statusCounts.completed}</strong></span><span><small>En cours</small><strong>${data.statusCounts.active}</strong></span></div></section>`;root.querySelectorAll("[data-open-project]").forEach(b=>b.onclick=()=>openProject(projects.find(p=>p.id===b.dataset.openProject)))};
        stats.querySelectorAll("select").forEach(s=>s.onchange=paint);stats.querySelector("#v66StatsCurrent").onclick=()=>{stats.querySelector("#v66StatsMonth").value=String(now.getMonth());stats.querySelector("#v66StatsYear").value=String(now.getFullYear());paint()};stats.querySelector("#v105Compare").onclick=renderCompare;paint();
      };
      const renderCompare=()=>{stats.innerHTML=`<section class="v105-compare-header"><div><h3>Comparer deux périodes</h3><p>Mois ou années complètes, avec les mêmes indicateurs.</p></div><button class="v66-btn" data-compare-back>← Vue générale</button></section><div class="v105-compare-pickers"><article><strong>Période A</strong><label>Année<select id="v105YearA">${years()}</select></label><label>Période<select id="v105MonthA">${months()}</select></label></article><article><strong>Période B</strong><label>Année<select id="v105YearB">${years()}</select></label><label>Période<select id="v105MonthB">${months()}</select></label></article></div><div id="v105CompareResults"></div>`;stats.querySelector("#v105YearA").value=String(now.getFullYear());stats.querySelector("#v105MonthA").value="all";stats.querySelector("#v105YearB").value=String(now.getFullYear()-1);stats.querySelector("#v105MonthB").value="all";const paint=()=>{const A=metrics("all",stats.querySelector("#v105MonthA").value,stats.querySelector("#v105YearA").value),B=metrics("all",stats.querySelector("#v105MonthB").value,stats.querySelector("#v105YearB").value),items=[["Heures réalisées",A.total,B.total,"h"],["Chantiers travaillés",A.projectCount,B.projectCount,""] ,["Salariés mobilisés",A.employees,B.employees,""] ,["Heures moy. / chantier",A.avgProject,B.avgProject,"h"],["Heures moy. / salarié",A.avgEmployee,B.avgEmployee,"h"],["Journées-personnes",A.workdays,B.workdays,""]];stats.querySelector("#v105CompareResults").innerHTML=`<section class="v105-compare-summary"><div><small>Période A</small><strong>${esc(A.period.label)}</strong></div><span>VS</span><div><small>Période B</small><strong>${esc(B.period.label)}</strong></div></section><div class="v105-compare-grid">${items.map(([label,a,b,u])=>`<article><small>${label}</small><div><strong>${u==="h"?fmt(a):Number(a).toLocaleString("fr-FR",{maximumFractionDigits:1})}</strong><span>${delta(a,b,u?" h":" ")}</span></div><p>contre ${u==="h"?fmt(b):Number(b).toLocaleString("fr-FR",{maximumFractionDigits:1})}</p></article>`).join("")}</div><section class="v105-two-col"><article class="v66-card"><div class="v105-card-head"><h3>Top 5 — ${esc(A.period.label)}</h3></div><div class="v105-ranking">${[...A.byProject.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5).map(([id,h],i)=>{const p=projects.find(x=>x.id===id);return p?`<div><b>${i+1}</b><span>${esc(p.name)}</span><strong>${fmt(h)}</strong></div>`:""}).join("")||'<div class="v66-empty">Aucune donnée.</div>'}</div></article><article class="v66-card"><div class="v105-card-head"><h3>Top 5 — ${esc(B.period.label)}</h3></div><div class="v105-ranking">${[...B.byProject.entries()].sort((a,b)=>b[1]-a[1]).slice(0,5).map(([id,h],i)=>{const p=projects.find(x=>x.id===id);return p?`<div><b>${i+1}</b><span>${esc(p.name)}</span><strong>${fmt(h)}</strong></div>`:""}).join("")||'<div class="v66-empty">Aucune donnée.</div>'}</div></article></section>`};stats.querySelectorAll("select").forEach(s=>s.onchange=paint);stats.querySelector("[data-compare-back]").onclick=renderGeneral;paint()};
