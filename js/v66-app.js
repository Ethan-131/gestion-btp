...
async function boot(db) {
  ...
  async function renderLeaves(root, monthOffset = 0) {
    ...
    root
      ...
      ?.addEventListener("click", () => leaveModal(false));
    try {
      const { data, error } = await db
        .from("leave_requests")
        .select(
      ...
      renderLeaveCalendar(
        ...
        requests,
        role,
        canCreate ? (date) => leaveModal(false, date) : null,
      );
