...
async function boot(db) {
  ...
  function projectTimeCategory(project){const today=dateKey(new Date());if(project.planned_start_date>today)return "upcoming";if(project.planned_end_date<today||["completed","archived"].includes(project.status))return "completed";return "active"}
  ...
  async function renderStats(root) {
    root.innerHTML ='<div class="v66-pagehead"><div><h1>Statistiques chantiers</h1><p>Analyse générale ou détail d’un chantier.</p></div></div><div class="v66-stats-view-tabs"><button type="button" class="active" data-stats-view="general">Vue générale</button><button type="button" data-stats-view="projects">Par chantier</button></div><div id="v66Stats"><div class="v66-card v66-empty">Calcul…</div></div>';
    try {
      const [{ data: projects, error: pe }, { data: sheets, error: se }] =
        await Promise.all([
      ...
      if (pe) throw pe;
      if (se) throw se;
      const entries=[];(sheets||[]).forEach(sheet=>(sheet.timesheet_days||[]).forEach(day=>(day.timesheet_sites||[]).forEach(site=>{if(site.project_id)entries.push({projectId:site.project_id,date:day.work_date,employeeId:sheet.employee_id,name:fullName(sheet.profiles)||sheet.profiles?.email||"Salarié",hours:Number(site.hours||0)})})));
      const metrics=(projectId="all",month="all",year="all")=>{const filtered=entries.filter(entry=>(projectId==="all"||entry.projectId===projectId)&&(year==="all"||Number(entry.date.slice(0,4))===Number(year))&&(month==="all"||Number(entry.date.slice(5,7))-1===Number(month))),byProject=new Map(),byEmployee=new Map(),buckets=new Map();filtered.forEach(entry=>{byProject.set(entry.projectId,(byProject.get(entry.projectId)||0)+entry.hours);const key=`${entry.projectId}:${entry.employeeId}`,person=byEmployee.get(key)||{projectId:entry.projectId,name:entry.name,hours:0};person.hours+=entry.hours;byEmployee.set(key,person);const date=new Date(`${entry.date}T12:00:00Z`),bucket=month==="all"?monthLabels[date.getUTCMonth()]:`Semaine ${isoWeekFromDate(date).week}`;buckets.set(bucket,(buckets.get(bucket)||0)+entry.hours)});return{total:filtered.reduce((sum,x)=>sum+x.hours,0),employees:new Set(filtered.map(x=>x.employeeId)).size,byProject,people:[...byEmployee.values()].sort((a,b)=>b.hours-a.hours),buckets}};
      const conductor=p=>(p.project_conductors||[]).map(item=>fullName(item.profiles)).filter(Boolean).join(", ")||"Aucun conducteur affecté";
      const openDrawer=(project,data,label="Toutes périodes")=>{const start=new Date(`${project.planned_start_date}T12:00:00`),end=new Date(`${project.planned_end_date}T12:00:00`),pct=Math.max(0,Math.min(100,((new Date()-start)/Math.max(1,end-start))*100)),people=data.people.filter(x=>x.projectId===project.id);root.innerHTML=`<article class="v66-project-stats-page"><button type="button" class="v66-btn v66-stats-back" data-stats-back>← Retour aux statistiques</button><header class="v66-project-stats-hero"><div><small>${esc(project.code)}</small><h1>${esc(project.name)}</h1><p>${fmtDate(project.planned_start_date)} → ${fmtDate(project.planned_end_date)}</p></div><span class="v66-pill ${projectTimeCategory(project)}">${projectCategoryLabels[projectTimeCategory(project)]}</span></header><p class="v66-help">${esc(label)}</p><div class="v66-project-stat-summary"><div><small>Conducteur de travaux</small><strong>${esc(conductor(project))}</strong></div><div><small>Heures réalisées</small><strong>${(data.byProject.get(project.id)||0).toLocaleString("fr-FR")} h</strong></div><div><small>Temps prévisionnel écoulé</small><strong>${pct.toFixed(0)} %</strong></div></div><section class="v66-card"><h2>Avancement dans le temps</h2><div class="v66-time-progress"><span style="width:${pct}%"></span></div><div class="v66-time-legend"><small>${fmtDate(project.planned_start_date)}</small><b>${pct.toFixed(0)} % de la période écoulée</b><small>${fmtDate(project.planned_end_date)}</small></div></section><section class="v66-card v66-project-workers"><h2>Temps passé par salarié</h2>${people.length?people.map((person,index)=>`<div class="${index>=3?"v66-worker-extra":""}" ${index>=3?"hidden":""}><span><b>${index+1}</b>${esc(person.name)}</span><strong>${person.hours.toLocaleString("fr-FR")} h</strong></div>`).join(""):'<p class="v66-help">Aucune heure enregistrée.</p>'}${people.length>3?'<button type="button" class="v66-link-btn" data-more-workers>Voir les autres salariés</button>':""}</section></article>`;root.querySelector("[data-stats-back]").onclick=()=>renderStats(root);root.querySelector("[data-more-workers]")?.addEventListener("click",e=>{const hidden=[...root.querySelectorAll(".v66-worker-extra")].some(row=>row.hidden);root.querySelectorAll(".v66-worker-extra").forEach(row=>row.hidden=!hidden);e.currentTarget.textContent=hidden?"Réduire":"Voir les autres salariés"});window.scrollTo({top:0,behavior:"smooth"})};
      const stats=root.querySelector("#v66Stats"),now=new Date();let currentView="general";
      const renderGeneral=()=>{stats.innerHTML=`<section class="v66-card v66-stats-filters"><label>Chantier<select id="v66StatsProject"><option value="all">Tous les chantiers</option>${projects.map(p=>`<option value="${p.id}">${esc(p.code)} — ${esc(p.name)}</option>`).join("")}</select></label><label>Mois<select id="v66StatsMonth"><option value="all">Toute l’année</option>${monthLabels.map((m,i)=>`<option value="${i}" ${i===now.getMonth()?"selected":""}>${m}</option>`).join("")}</select></label><label>Année<select id="v66StatsYear">${Array.from({length:81},(_,i)=>2020+i).map(y=>`<option value="${y}" ${y===now.getFullYear()?"selected":""}>${y}</option>`).join("")}</select></label><button type="button" class="v66-btn" id="v66StatsCurrent">Période actuelle</button></section><div id="v66StatsOverview"></div>`;const paint=()=>{const projectId=stats.querySelector("#v66StatsProject").value,month=stats.querySelector("#v66StatsMonth").value,year=stats.querySelector("#v66StatsYear").value,data=metrics(projectId,month,year),ranking=[...data.byProject.entries()].sort((a,b)=>b[1]-a[1]),max=ranking[0]?.[1]||1,label=`${month==="all"?"Année":monthLabels[Number(month)]} ${year}`;stats.querySelector("#v66StatsOverview").innerHTML=`<div class="v66-stats-kpis"><div><small>Heures réalisées</small><strong>${data.total.toLocaleString("fr-FR")} h</strong></div><div><small>Chantiers concernés</small><strong>${data.byProject.size}</strong></div><div><small>Salariés mobilisés</small><strong>${data.employees}</strong></div></div><section class="v66-card v66-stats-ranking"><h2>${projectId==="all"?"Répartition par chantier":"Évolution de la période"}</h2>${projectId==="all"?(ranking.length?ranking.map(([id,hours])=>{const p=projects.find(x=>x.id===id);return p?`<button type="button" data-open-project="${id}"><span><b>${esc(p.code)} — ${esc(p.name)}</b><i style="width:${hours/max*100}%"></i></span><strong>${hours.toLocaleString("fr-FR")} h</strong></button>`:""}).join(""):'<div class="v66-empty">Aucune heure sur cette période.</div>'):[...data.buckets.entries()].map(([name,hours])=>`<div class="v66-stats-bucket"><span>${esc(name)}</span><strong>${hours.toLocaleString("fr-FR")} h</strong></div>`).join("")||'<div class="v66-empty">Aucune heure sur cette période.</div>'}</section>`;stats.querySelectorAll("[data-open-project]").forEach(button=>button.onclick=()=>openDrawer(projects.find(p=>p.id===button.dataset.openProject),data,label));if(projectId!=="all"){const p=projects.find(x=>x.id===projectId);if(p)stats.querySelector(".v66-stats-ranking h2").onclick=()=>openDrawer(p,data,label)}};stats.querySelectorAll("select").forEach(select=>select.onchange=paint);stats.querySelector("#v66StatsCurrent").onclick=()=>{stats.querySelector("#v66StatsMonth").value=String(now.getMonth());stats.querySelector("#v66StatsYear").value=String(now.getFullYear());paint()};paint()};
      const renderProjectsView=()=>{let category="active",query="";stats.innerHTML='<div class="v66-project-category-tabs" id="v66StatsCategoryTabs"></div><input class="v66-search" id="v66StatsSearch" placeholder="Code ou nom du chantier…"><div class="v66-stats-project-list" id="v66StatsProjects"></div>';const paint=()=>{const rows=projects.filter(p=>projectTimeCategory(p)===category&&smartSearchMatch(`${p.code} ${p.name}`,query));stats.querySelector("#v66StatsCategoryTabs").innerHTML=["active","upcoming","completed"].map(c=>`<button type="button" class="${c===category?"active":""}" data-category="${c}">${projectCategoryLabels[c]} <b>${projects.filter(p=>projectTimeCategory(p)===c).length}</b></button>`).join("");stats.querySelectorAll("[data-category]").forEach(b=>b.onclick=()=>{category=b.dataset.category;paint()});stats.querySelector("#v66StatsProjects").innerHTML=rows.length?rows.map(p=>`<button type="button" class="v66-stat-project-row" data-project="${p.id}"><span><strong>${esc(p.code)} — ${esc(p.name)}</strong><small>${fmtDate(p.planned_start_date)} → ${fmtDate(p.planned_end_date)} · ${esc(conductor(p))}</small></span><span class="v66-pill ${projectTimeCategory(p)}">${projectCategoryLabels[projectTimeCategory(p)]}</span><i>›</i></button>`).join(""):'<div class="v66-card v66-empty">Aucun chantier trouvé.</div>';stats.querySelectorAll("[data-project]").forEach(b=>b.onclick=()=>openDrawer(projects.find(p=>p.id===b.dataset.project),metrics(b.dataset.project),"Toutes périodes"))};stats.querySelector("#v66StatsSearch").oninput=e=>{query=e.target.value;paint()};paint()};
      ...
      const intended=routeIntent?.projectId?projects.find(p=>p.id===routeIntent.projectId):null;routeIntent=null;renderGeneral();if(intended)openDrawer(intended,metrics(intended.id),"Toutes périodes");
    } catch (e) {
  ...
  async function renderNativeCurrentTimesheet(panel,year,week){
    ...
    if(navigator.onLine){
      const[projectResult,sheetResult,establishmentResult]=await Promise.all([
        db.from("projects").select("id,code,name,status,it_km_plaisance,it_km_salies").neq("status","archived").order("code"),
      ...
      if(projectResult.error)throw projectResult.error;if(sheetResult.error)throw sheetResult.error;if(establishmentResult.error)throw establishmentResult.error;
