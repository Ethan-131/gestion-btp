...
if ("serviceWorker" in navigator && location.protocol.startsWith("http")) {
  navigator.serviceWorker.register("./sw.js?v=117", { updateViaCache: "none" }).then(reg=>reg.update()).catch(()=>{});
}
  ...
  async function renderStats(root) {
    ...
    try{
      const [{data:projects,error:pe},{data:sheets,error:se}]=await Promise.all([
        db.from("projects").select("*,project_conductors(conductor_id,profiles!project_conductors_conductor_id_fkey(first_name,last_name))").order("code"),
        db.from("timesheets").select("employee_id,status,profiles!timesheets_employee_id_fkey(first_name,last_name,email),timesheet_days(work_date,timesheet_sites(project_id,hours))")
      ]);if(pe)throw pe;if(se)throw se;
      const entries=[];
      (sheets||[]).forEach(sheet=>(sheet.timesheet_days||[]).forEach(day=>{
        const daySites=(day.timesheet_sites||[]).filter(site=>site.project_id);
      ...
      const metrics=(projectId="all",month="all",year=new Date().getFullYear(),cutoffMonthDay=null)=>{
        ...
        const total=filtered.reduce((s,x)=>s+x.hours,0),employees=byEmployee.size,projectCount=byProject.size;
        ...
        return{period,total,employees,projectCount,workdays:employeeDays.size,activeDays:activeDays.size,avgProject:projectCount?total/projectCount:0,avgEmployee:employees?total/employees:0,byProject,people:[...byProjectEmployee.values()].sort((a,b)=>b.hours-a.hours),employeeRanking:[...byEmployee.values()].sort((a,b)=>b.hours-a.hours),buckets:orderedBuckets,statusCounts:{started:projects.filter(p=>inPeriod(String(p.planned_start_date||""),period)).length,completed:projects.filter(p=>projectTimeCategory(p)==="completed"&&inPeriod(String(p.planned_end_date||""),period)).length,active:projects.filter(p=>projectTimeCategory(p)==="active"&&projectOverlaps(p,period)).length}};
      };
